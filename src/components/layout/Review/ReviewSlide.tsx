...
interface ReviewSliderProps {
  reviews: ReviewData[];
  ratingData: RatingCalculation;
  ratingHeader: HeaderData | null;
}
...
const ReviewSlider = ({
  ...
  console.log("Rating header",ratingHeader);
  ...
  // If no reviews are passed, don't render the component.
      ...
      <div className="flex flex-col md:flex-row md:justify-between gap-6">
        <div className="flex flex-col space-y-2">
          <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold">
            {ratingHeader?.header || "Read Real Experiences From Our Travelers"}
          </h1>
          <p className="py-3 text-sm sm:text-base">
            {ratingHeader?.title ||
              "From Arctic ice to tropical jungles – here's what our guests say about our European and global wildlife trips."}
          </p>
        </div>
          ...
          <div className="flex gap-2 items-center py-3 px-4 bg-dark my-2.5 rounded-lg text-gray-800 text-sm font-medium">
            <Image
              ...
              onError={(e) => {
                ...
                target.src = "/fallback-star.png";
              }}
            />
            <p>{ratingData?.average?.toFixed(1) || "5.0"}/5 Average Rating</p>
          </div>
          <div className="flex gap-2 items-center py-3 px-4 bg-dark my-2.5 rounded-lg text-gray-800 text-sm font-medium">
            <Image
              src={assets.badge}
              ...
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                target.src = "/fallback-badge.png";
              }}
            />
            <p>Based on {ratingData?.count || "100+"} verified reviews</p>
          </div>
          <div className="flex gap-2 items-center py-3 px-4 bg-dark my-2.5 rounded-lg text-gray-800 text-sm font-medium">
            <Image
              src={assets.badge2}
              ...
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                target.src = "/fallback-badge2.png";
              }}
            />
