...
import { getExpeditionAntarcticaSeo } from "@/api/seo/seoTitle";
...
export async function generateMetadata({ searchParams }: TripsListPageProps): Promise<Metadata> {
  const params = (await Promise.resolve(searchParams)) ?? ({} as SearchParams);
  const lang = getParam(params, "lang") || "en";

  try {
    const seoData = await getExpeditionAntarcticaSeo(lang);
    const title = (seoData as any)?.data?.data?.title || "Expedition Antarctica";
    const description = (seoData as any)?.data?.data?.description || "Explore Antarctica with our expedition cruises.";
    ...
    return {
      ...
      description: "Explore Antarctica with our expedition cruises.",
    };
  }
}
...
const TripsList = async ({ searchParams }: TripsListPageProps) => {
  ...
  const [allTrips, initialData, seoData] = await Promise.all([
    ...
    getExpeditionAntarcticaSeo(lang),
  ]);
        ...
        <div className="text-gray-600 text-lg leading-relaxed container mx-auto text-start prose prose-lg max-w-none">
          {parse(
            (seoData as any)?.data?.data?.description ||
              "Explore Antarctica with our expedition cruises."
          )}
        </div>
      </div>
    </div>
