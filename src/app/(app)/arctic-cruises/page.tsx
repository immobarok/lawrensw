...
import { getArcticCruisesSeo } from "@/api/seo/seoTitle";
...
export async function generateMetadata({
  ...
  try {
    const seoData = await getArcticCruisesSeo(lang);
    const title = (seoData as any)?.data?.data?.title || "Arctic Cruise";
    const description =
      (seoData as any)?.data?.data?.description ||
      "Explore the Arctic with our cruise packages.";
    ...
    return {
      title: "Arctic Cruise",
      description: "Explore the Arctic with our cruise packages.",
    };
  }
}
...
const TripsList = async ({ searchParams }: TripsListPageProps) => {
  ...
  if (destination) requestFilters.destinations = destination;
  if (startDate) requestFilters.departure_date = startDate;
  ...
  if (ship) requestFilters.ship_name = ship;
  if (shipSize) requestFilters.shipSize = shipSize;

  const [allTrips, initialData, seoData] = await Promise.all([
    fetchAllTrips(),
    getAllTripsWithPagination(requestFilters),
    getArcticCruisesSeo(lang),
  ]);
